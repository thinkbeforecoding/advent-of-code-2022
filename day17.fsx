...
module Rock = 
    let create shape =
        { Shape = shape; Width = shape[0].Length; Height = shape.Length} 
...
module Input =
    ...
    let next (input: Input) = {input with Index = (input.Index + 1) % input.Moves.Length}
    ...
    let rockLines =
        ...
        |> List.map (fun y ->
            if Chamber.isInLine y chamber then
                ...
                String(' ', rock.X) + FallingRock.line y rock + String(' ', 7 - rock.X - rock.Rock.Width)
...
let step t =
    ...
    let moved =
        if touch lateral t.Chamber then
            t.Rock
        else
            lateral
