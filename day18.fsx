...
let dim points =
    (points |> Array.fold (fun (xa,ya,za) (x,y,z) -> max xa x, max ya y, max za z) (0,0,0))
    ++ (3,3,3)
...
let inside (x,y,z) (space: _[,,]) =
    x >= 0 && x < space.GetLength(0)
    && y >= 0 && y < space.GetLength(1)
    && z >= 0 && z < space.GetLength(2)

let get (x,y,z) (space: _[,,]) =
    space[x,y,z]

let set (x,y,z) v (space: _[,,]) =
    space[x,y,z] <- v
...
let space2 = 
    ...
    for (x,y,z) in points do
        s[x+1,y+1,z+1] <-  1
    fill s [0,0,0; dx-1,0,0; 0, dy-1,0; dx-1,dy-1,0; 0,0,dz-1; dx-1,0,dz-1; 0, dy-1,dz-1; dx-1,dy-1,dz-1  ]
[ for y in 0 .. dy-1 do
    for x in 0.. dx-1 do
